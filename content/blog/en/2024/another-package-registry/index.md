...
---
When I was on FOSDEM this year in [the talk of Leo Kettmeir](https://fosdem.org/2024/schedule/event/fosdem-2024-3665-building-your-own-javascript-runtime-with-rust/) from Deno, he talked about closed beta access to their new package registry — [JSR](https://jsr.io/) . I kept not so much attention to it. Recently, with announcement of public beta access, discussions around the Internet started to appear.

First of all, JSR is drived by [deno community](https://deno.com/blog/jsr_open_beta). So I think it's obvious motivation to create package manager and registry for Deno. Yes, JSR is positioned as a package registry not only for Deno, but for usage with Node.js, browsers, etc through `npm` and `yarn`. But I think it's not the main goal anyway. At least reply of JSR developer [Luca Casonato](https://twitter.com/lcasdev) doesn't give me any confidence in it:
